...
public class AuthenticationService : IAuthService
{
    ...
    public async Task<ApiResponse<AuthenticationResponse>> LoginAsync(LoginRequest request, string ipAddress, CancellationToken cancellationToken = default)
    {
        var user = await _userManager.FindByEmailAsync(request.Email);
        if (user == null)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Invalid email or password", 401);

        if (!user.IsActive)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Account is inactive", 403);
        ...
        if (!result.Succeeded)
        {
            if (result.IsLockedOut)
                return ApiResponse<AuthenticationResponse>.FailureResponse("Account is locked out", 403);
    ...
    public async Task<ApiResponse<AuthenticationResponse>> RegisterAsync(RegisterRequest request, CancellationToken cancellationToken = default)
    {
        var existingUser = await _userManager.FindByEmailAsync(request.Email);
        if (existingUser != null)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Email already exists", 400);

        var existingUserName = await _userManager.FindByNameAsync(request.UserName);
        if (existingUserName != null)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Username already exists", 400);
        ...
        if (!result.Succeeded)
        {
            ...
            return ApiResponse<AuthenticationResponse>.FailureResponse("Registration failed", 400, errors);
        }
        ...
        return ApiResponse<AuthenticationResponse>.SuccessResponse(response, "Registration successful", 201);
    }

    public async Task<ApiResponse<AuthenticationResponse>> RefreshTokenAsync(RefreshTokenRequest request, string ipAddress, CancellationToken cancellationToken = default)
    {
        var principal = _jwtTokenService.GetPrincipalFromExpiredToken(request.AccessToken);
        if (principal == null)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Invalid token", 401);
        ...
        var storedTokenResult = await _refreshTokenRepository.GetByTokenAsync(request.RefreshToken, cancellationToken);
        if (storedTokenResult.IsFailure)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Invalid refresh token", 401);

        var storedToken = storedTokenResult.Value;
        if (storedToken == null || !storedToken.IsActive || storedToken.UserId != userId || storedToken.IsExpired)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Invalid refresh token", 401);
        ...
        var updateResult = await _refreshTokenRepository.UpdateAsync(storedToken, cancellationToken);
        if (updateResult.IsFailure)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Failed to update token", 500);
        ...
        var updateReplacedResult = await _refreshTokenRepository.UpdateAsync(storedToken, cancellationToken);
        if (updateReplacedResult.IsFailure)
            return ApiResponse<AuthenticationResponse>.FailureResponse("Failed to update token", 500);
        ...
        return ApiResponse<AuthenticationResponse>.SuccessResponse(response, "Token refreshed");
    }
